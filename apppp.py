...
# Виносимо в функцію, щоб легко перевіряти підключення
...
def load_items_from_db():
    ...
    try:
        ...
        if response.data:
            ...
            # Це не помилка, просто база може бути порожньою
...
# Ця функція не кешується окремо
...
# --- Головна сторінка (може бути просто заголовок або вступ) ---
st.title("📊 Програма обліку товарів")
st.write("Оберіть потрібний розділ на бічній панелі зліва.")

# --- Ініціалізація стану додатку (якщо щось потрібно зберігати між сторінками) ---
# Ці стани тепер будуть використовуватися всередині сторінок
